...
public:
    ...
    Arbres(std::string strShader, vec3 c, Transform tr, Entity* p); ///Constructeur couleur
	Object3D* get_tree(int i) { return arbres[i]; } ///Renvoie l'arbre d'indice i
    vector<Object3D*> make_trees(std::string strShader, vec3 c, Transform tr, Entity* p, float px, float pz, float r, int nb_arbre, float ra); ///Créer la liste d'arbres à positions aléatoires
};
