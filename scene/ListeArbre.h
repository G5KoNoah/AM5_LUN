...
public:
    ...
    Arbres(std::string strShader, vec3 c, Transform tr, Entity* p, Terrain* terrain, float px, float pz, float r, int nb_arbre, float ra); ///Constructeur couleur
	Object3D* get_tree(int i) { return arbres[i]; } ///Renvoie l'arbre d'indice i
	int get_size() { return arbres.size(); } ///Renvoie le nombre d'arbres
